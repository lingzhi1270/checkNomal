...
- (void)setup {
    ...
    self.textView = [[_WBGTextView alloc] initWithFrame:CGRectMake(0, kTopOffset, SCREENWIDTH, SCREENHEIGHT - kTopOffset)];
...
- (void)addNewText:(NSString *)text
{
    ...
    view.text = text;
    view.center = [self.editor.drawingView.superview convertPoint:self.editor.drawingView.center toView:self.editor.drawingView];
    view.userInteractionEnabled = YES;
    [self.editor.drawingView addSubview:view];
    
    view.textToolViewDidTap = ^(BOOL isHidden) {
        self.textToolDidTap(isHidden);
    };
    
    view.textToolViewDidPan = ^(BOOL isHidden) {
        self.textToolDidPan(isHidden);
    };
    
    view.textToolViewDidPinch = ^(BOOL isHidden) {
        self.textToolDidPinch(isHidden);
    };
    
    view.textToolViewDidRotate = ^(BOOL isHidden) {
        self.textToolDidRotate(isHidden);
    };
