...
- (void)viewDidPan:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan ||
        ...
        CGRect rectCoordinate = [piece.superview convertRect:piece.frame toView:self.textTool.editor.drawingView.superview];
        if (!CGRectIntersectsRect(CGRectInset(self.textTool.editor.drawingView.frame, 30, 30), rectCoordinate)) {
...
- (void)viewDidRotation:(UIRotationGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan ||
        ...
        [self layoutSubviews];
        self.textToolViewDidRotate(YES);
        //取消当前
        [self.textTool.editor resetCurrentTool];
    } else if (recognizer.state == UIGestureRecognizerStateEnded ||
               recognizer.state == UIGestureRecognizerStateFailed ||
               recognizer.state == UIGestureRecognizerStateCancelled) {
        self.textToolViewDidRotate(NO);
    }
}
