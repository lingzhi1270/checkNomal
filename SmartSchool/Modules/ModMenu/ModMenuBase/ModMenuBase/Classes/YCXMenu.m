...
#define kTitleFont  [UIFont systemFontOfSize:14.0]
...
- (UIView *)mkContentView {
    ...
    for (YCXMenuItem *menuItem in _menuItems) {
        const CGFloat itemHeight = 44.0f;
        const CGFloat itemWidth = ((!menuItem.enabled && !menuItem.image) ? 80 : maxImageWidth + 80) + kMarginX * 4;
    ...
    const CGFloat titleX = kMarginX * 2 + maxImageWidth+8;
...
//            titleLabel.textAlignment = menuItem.alignment;
        ...
        if (menuItem.image) {
            ...
            [imageView setImageWithURL:[NSURL URLWithString:menuItem.image] options:YYWebImageOptionProgressiveBlur];
        }
        
        if (menuItem.unreadNumber) {
            const CGRect numLabelFrame = {titleX + titleWidth-10, [YCXMenu menuItemMarginY], 20, 20};
            ...
            numLabel.text = [NSString stringWithFormat:@"%ld",(long)menuItem.unreadNumber];
...
+ (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItemsData:(NSArray *)menuItems selected:(YCXMenuSelectedItem)selectedItem {
    [[self sharedMenu] showMenuInView:view fromRect:rect menuItems:[self menuData] selected:selectedItem];
}

+ (NSArray *)menuData
{
    NSArray *items = [@[[YCXMenuItem menuItem:@"Menu"
                       image:[UIImage imageNamed:@"ic_friend_connect"]
                       tag:99
                       unreadNumber:nil
                       userInfo:nil],
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(YCXMenuSelectedItem)selectedItem {
    ...
    [_menuView showMenuInView:view fromRect:rect menuItems:[self menuArray] selected:selectedItem];
    self.isShow = YES;
}
...
+(UIColor *)selectedColor {
    return gSelectedColor?gSelectedColor:kTintColor;
}
